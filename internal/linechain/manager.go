...
type Document struct {
	...
	SingBoxBinary          string   `json:"singbox_binary,omitempty"`
...
func validateDocument(d Document) error {
	...
	wantSidecar := filepath.Join(filepath.Dir(configDir), "lattice-metadata.json")
	if filepath.Clean(d.SidecarPath) != wantSidecar {
		return fmt.Errorf("sidecar path must be %s", wantSidecar)
	}
...
func (m *Manager) checkRestartVerify(ctx context.Context, d Document) error {
	bin := strings.TrimSpace(d.SingBoxBinary)
	if bin == "" {
		bin = "sing-box"
	}
	args := d.CheckArgs
	if len(args) == 0 {
		args = []string{"check", "-C", d.ConfigDir}
	}
	if out, err := m.run(ctx, bin, args...); err != nil {
		return fmt.Errorf("sing-box check failed: %s", bounded(out))
	}
	restart := d.RestartCommand
	if len(restart) == 0 {
		restart = []string{"systemctl", "restart", "sing-box"}
	}
	if out, err := m.run(ctx, restart[0], restart[1:]...); err != nil {
		return fmt.Errorf("sing-box restart failed: %s", bounded(out))
	}
	verify := d.VerifyCommand
	if len(verify) == 0 {
		verify = []string{"systemctl", "is-active", "--quiet", "sing-box"}
	}
...
func validateParents(path string) error {
	...
	if !info.IsDir() || info.Mode()&os.ModeSymlink != 0 || !ownedPath(info) {
		return fmt.Errorf("artifact parent must be a real directory owned by the agent user")
	}
	return nil
}
